...
######## SHED: +1 B1
shed.PK = rep(0,33); shed.PK[which(varnames=='B1')] = 1
...
trans = rep(0,33); trans[which(varnames=='B1')] = -1; trans[which(varnames=='B2')] = 1
die = rep(0,33); die[which(varnames=='B2')] = -1
...
onestep = function(x,pars){
  ...
  rates = c(
    ...
    mig.S.1a = omega*(1-popA)*S1a,
    mig.S.1b = omega*popA*S1b,
...
simul.fn = function(x,params,maxstep,tmax){
  ...
  while (j<=maxstep){
    ...
    if (y[1]=='no transmission'){
      return(matrix(9999,tmax+1,7))
    } else{
      if (floor(y['time'])>floor(x['time'])){
        out[ceiling(x['time']),] = c(floor(y['time']),x['Ya1a'],x['Y1a'],x['Ya1b'],x['Y1b'],x['Ya2'],x['Y2'])
      }
      x = y
      names(x) = c('time',varnames)
    }
...
set.seed(10100)
load(file='~/chol1.mcmc.Rdata')
pars = c(1/1.55,1/5.09,1/3.32,1/1.77,100,1,0.1,0.025,9923243,879644,9043599,1,1/30,8.58)
nsims = 100
Tmax = 30
runs2 = array(NA,dim=c(nsims,Tmax+1,7))

for (l in 29:nsims){
  par = c(pars,state1[sample(2001:50000,1),,sample(1:3,1)])
  for (z in 1:length(parnames)){
    assign(parnames[z],par[z])
  }
  init = c(1,popA*Nart,(1-popA)*Nart,Noth,rep(0,29))
  runs2[l,,] = simul.fn(x=c(0,init),params=par,maxstep=1e7,tmax=Tmax)
